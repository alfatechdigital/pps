...
function dompdf_usage() {
  $default_paper_size = defined('DOMPDF_DEFAULT_PAPER_SIZE') ? DOMPDF_DEFAULT_PAPER_SIZE : 'A4';
...
switch ( $sapi ) {
 ...
 case "cli":
  ...
  if ( isset($opts["l"]) ) {
    ...
    foreach (array_keys((array) CPDF_Adapter::$PAPER_SIZES) as $size)
  ...
  if ( isset($opts["p"]) )
    ...
    $paper = defined('DOMPDF_DEFAULT_PAPER_SIZE') ? DOMPDF_DEFAULT_PAPER_SIZE : 'A4';
 ...
 default:
  ...
  if(($file_parts['protocol'] == '' || $file_parts['protocol'] === 'file://')) {
    $file = realpath($file);
    if ( strpos($file, defined('DOMPDF_CHROOT') ? DOMPDF_CHROOT : '/') !== 0 ) {
...
if ( $_dompdf_show_warnings ) {
  ...
  foreach ($_dompdf_warnings as $msg)
    echo $msg . "\n";
  echo $dompdf->get_canvas()->get_messages();
  flush();
}
...
//     throw new DOMPDF_Exception("'$outfile' is not writable.");
  if ( strtolower(defined('DOMPDF_PDF_BACKEND') ? DOMPDF_PDF_BACKEND : 'CPDF') === "gd" )
  ...
  if ( strpos($outfile, defined('DOMPDF_CHROOT') ? DOMPDF_CHROOT : '/') !== 0 )
